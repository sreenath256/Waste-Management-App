...
const RegionalUsers = () => {
  ...
  useEffect(() => {
    ...
    axiosClient
      .get("/user/regional/:calicut")
      .then((response) => {
        setUsers(response.data.data);
        //   console.log(response);; // Assuming the response.data contains an array of user objects
      })
      .catch((error) => {
  ...
  return (
    <>
      <header className="flex h-14 lg:h-[60px] items-center gap-4 border-b bg-gray-100/40 px-6 dark:bg-gray-800/40">
        ...
        {/* <Button size="sm"><Link to="/new-user">New User</Link></Button> */}
      </header>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Pincode</TableHead>
            ...
            <TableBody>
              {users.map((user, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {user.isSubscribed ? <><h3>Yes</h3> </> : <> <h3>No</h3></>}
                  </TableCell>
