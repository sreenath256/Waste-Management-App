...
const userController = {
  ...
  updateById: async (req, res) => {
    try {
      const { firstname, email, district, pincode, address, phone } = req.body;

      const updatedUser = await User.findOneAndUpdate(
        { _id: req.params.id },
        { firstname, email, district, pincode, address, phone },
        { new: true }
      );
      if (updatedUser) {
        res.json({
  ...
  subscribeById: async (req, res) => {
    ...
    try {
      ...
      const updatedUser = await User.findOneAndUpdate(
        { _id: req.params.id },
        { firstname, email, district, pincode, address, phone, isSubscribed },
        { new: true }
      );
      if (updatedUser) {
        ...
        await newBin.save();
        res.status(200).json({ msg: "User Subscribed" });
      } else {
        res.status(404).json({ msg: "User not found" });
      }
    } catch (error) {
